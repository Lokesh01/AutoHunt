...
import CarListsLoading from "./car-listing-loading";
...
const CarsListing = () => {
  ...
  const {
    ...
    data: result,
    error,
  } = useFetch(getFilteredCars);

  //fetch cars when filter change
  useEffect(() => {
    fetchCars({
      search,
      make,
  ...
  }, [page, currentPage, router, searchParams]);
  ...
  uniquePageNumbers.forEach((pageNumber) => {
    if (pageNumber - lastPageNumber > 1) {
      ...
      paginationItems.push(
        <PaginationItem key={`ellipsis-${lastPageNumber}`}>
          <PaginationEllipsis />
        </PaginationItem>
      );
    }
